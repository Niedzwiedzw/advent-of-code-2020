...
console.log(
  _(input)
    .split("\n")
    .map((line) => line.split(":"))
    .map(([ruleStr, password]) => [ruleStr.split(" "), password])
    .map(([[boundries, character], password]) => [
      dbg([boundries.split("-"), character]),
      password,
    ])
    .map(([[[min, max], character], password]) => dbg([
      parseInt(min),
      parseInt(max),
      password.split("").filter((c) => c === character).length,
    ]))
